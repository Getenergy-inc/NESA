...
const messages = [
  {
    ...
    message:`
      Dear Changemakers of Africa and the World,
...
const PopupSlider = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 z-[999] flex items-center mx-auto justify-center"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      >
        <motion.div
          className="bg-white w-[50%] rounded-xl shadow-lg p-4 text-center relative"
        ...
        >
          ...
          <div className=" min-h-[48px] transition-all duration-500">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="font-xl text-bold mb-2 text-[#ea580c]">{messages[current].title}</div>
                <div className="text-[#1a140b] text-sm">{messages[current].message}</div>
              </motion.div>
            </AnimatePresence>
          </div>
