            ...
            {new Array(3).fill(null).map((_, id) => (
              <p key={id} className={`py-1 ${styles["head-scroll-text"]}`}>
                <span className="text-sm text-center font-medium text-gray-800">
                  <span className="text-[#F0B561] font-bold">NESA TV</span>{" "}
                  <strong>ANNOUNCEMENT</strong> - Nomination Starts from March
                  15th, 2024 , Get ready to nominate your Education Champion!
                </span>
              </p>
            ))}
          </motion.div>
            ...
            {Array.from({ length: categoryData.length }).map((_, id) => (
              <div
                ...
                className={`w-3 h-3  rounded-full cursor-pointer ${
                  id === 0 ? "bg-primaryGold" : "bg-gray-300"
                }`}
              ></div>
            ))}
          </div>
