    ...
    <>
      <header className="md:min-h-[35rem] min-h-screen relative">
        ...
        <div className="absolute top-0 left-0 w-full h-full bg-black/80 flex items-center justify-center pt-10">
          <div className="space-y-5 text-white md:max-w-4xl container mx-auto text-center mt-5">
            <h1
              className={`${raleway.className} font-bold md:text-5xl text-4xl text-deepGold`}
            >
              {" "}
              NESA MEDIA
            </h1>
            <div className="flex items-center gap-4 justify-center">
              {["CATEGORIES VIDEOS", "STREETS OF AFRICA", "PICTURES"].map(
                (_, id) => (
      ...
      <main className="py-20">
        <div className="container">
          ...
          <div className="mt-2 grid md:grid-cols-3 gap-2">
            {Array.from({ length: 6 }).map((_, id) => (
              <div
                key={id}
                className="grid items-center bg-[#fff5e0] rounded-lg overflow-hidden"
              >
                ...
                <div className="space-y-4 p-8">
                  ...
                  <p className="line-clamp-5">
                    The NGO Educational Champion of the Decade Award is
                    established to honor and recognize the pivotal role of
                    Nigerian indigenous NGOs and civil society organizations in
                    championing the cause of &apos;Education for All&apos;. The
                    2023 award cycle specifically acknowledges those entities
                    that have excelled in aligning with the objectives of civil
                    society organizations, including continuous learning, adult
                    education, environmental education, and alignment with
                ...
                >
                  ...
                  <div className="p-2">
                    <p className="text-center">
                      NIGERIA: THE GIANT OF AFRICA DOCUMENTARY
                    </p>
                  </div>
                </div>
              ))}
        ...
        <div className="container mt-16 space-y-8 relative">
          ...
          {/* left arrow */}
          <ChevronLeft
            className="cursor-pointer text-deepGold absolute top-[44%] -left-8"
            size={50}
          />
                ...
                <div className="md:max-h-[15rem] max-h-[10rem] overflow-hidden rounded-md">
                  <Image
                    src={`/images/media/${
                      id >= 4
                        ? [1, 2, 3, 4][
                            Math.floor(Math.random() * [1, 2, 3, 4].length)
                          ]
                        : id + 1
                    }.png`}
                    alt="image"
          ...
          {/* right arrow */}
          <ChevronRight
            className="cursor-pointer text-deepGold absolute top-[44%] -right-8"
            size={50}
          />
            ...
            {Array.from({ length: 4 }).map((_, id) => (
              <div
                ...
                className={`w-2 h-2 rounded-full duration-200 border ${
                  id === 2
                    ? "bg-deepGold border-deepGold px-3"
                    : "bg-transparent border-darkGold/50"
                }`}
              ></div>
            ))}
          </div>
