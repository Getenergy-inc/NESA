...
export interface INRCVolunteerModel extends Model<INRCVolunteer> {
  getLeaderboard(type: 'weekly' | 'monthly' | 'allTime', limit: number): Promise<INRCVolunteer[]>;
...
NRCVolunteerSchema.index({ userId: 1 });
NRCVolunteerSchema.index({ email: 1 });
...
NRCVolunteerSchema.statics.getLeaderboard = async function(
  ...
  return this.find({ status: 'ACTIVE' })
    ...
    .limit(limit)
    .select('fullName displayName country nomineesUploaded agcEarned rank level badge');
};

NRCVolunteerSchema.statics.getVolunteerStats = async function(volunteerId: string) {
...
NRCVolunteerSchema.methods.updateStats = async function() {
  this.completionRate = this.targetNominees > 0 
...
export default mongoose.models.NRCVolunteer as INRCVolunteerModel || 
  mongoose.model<INRCVolunteer, INRCVolunteerModel>('NRCVolunteer', NRCVolunteerSchema);
